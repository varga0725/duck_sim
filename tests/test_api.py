...
def test_camera_info_mock_contract(client):
...
def test_camera_info_webcam_marks_uncalibrated(client, monkeypatch):
...
def test_camera_info_real_fpv_contract(client, monkeypatch):
    from duck_agent_sim.bridge import api
...
def test_api_command_state_stability_uses_command_safety_thresholds(client):
    response = client.get("/health")
    if response.json()["sim_mode"] == "mock":
        client.post("/reset")
        payload = {
...
def test_api_walk_square_scenario(client):
    response = client.get("/health")
    if response.json()["sim_mode"] == "mock":
