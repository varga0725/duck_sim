...
def test_onnx_model_loading_and_shape():
    assert DUCK_ONNX_MODEL_PATH != "", (
        "Default DUCK_ONNX_MODEL_PATH should be configured"
    )
    assert os.path.exists(DUCK_ONNX_MODEL_PATH), (
        f"Model file must exist at {DUCK_ONNX_MODEL_PATH}"
    )
    ...
    action = session.run(
        None,
        {input_meta.name: np.zeros((1, OBSERVATION_SIZE), dtype=np.float32)},
    )[0]
...
def test_real_simulator_onnx_observation_shape_when_mujoco_available():
    ...
    try:
        ...
        assert sim._onnx_session is not None, (
            "ONNX InferenceSession should be initialized"
        )
